...
def search(request):
    ...
    for prod in Product.objects.raw('SELECT 1 id,  title FROM products'):
        payload.append(prod.title)

    # print(request.GET)
    filtered_list = [tup for tup in payload if request.GET['title'].lower() in tup.lower()]

    # unzipped_object = zip(*filtered_list)
    # unzipped_list = list(unzipped_object )

    return JsonResponse({'status' : 200 , 'data':filtered_list})
